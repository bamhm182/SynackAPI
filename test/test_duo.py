...
class DuoTestCase(unittest.TestCase):
    ...
    def test_answer_transaction_uses_state_overrides(self):
        """Should use state Duo credentials instead of database values"""
        self.duo._generate_push_signature = MagicMock(return_value='Basic sig')
        self.duo._db.duo_akey = 'db_akey'
        self.duo._db.duo_host = 'db.duo.com'
        self.state.duo_akey = 'state_akey'
        self.state.duo_host = 'state.duo.com'
        self.duo._answer_transaction({'urgid': 'txn123'}, 'approve')
        call_args = self.duo._api.request.call_args
        self.assertIn('https://state.duo.com/', call_args[0][1])
        self.assertEqual(call_args[1]['data']['akey'], 'state_akey')
        self.assertEqual(call_args[1]['headers']['host'], 'state.duo.com')

    def test_answer_transaction_verified_push_code(self):
        """Should include verified_push_code in the signed body when provided"""
        captured = {}

        self.duo._generate_push_signature = MagicMock(
            side_effect=lambda method, path, now, data: captured.setdefault('data', dict(data)) or 'Basic sig')
        self.duo._db.duo_akey = 'akey'
        self.duo._db.duo_host = 'api.duosecurity.com'
        self.duo._answer_transaction({'urgid': 'txn123'}, 'approve',
    ...
    def test_configure_mfa_push_new(self):
        """Should call get_device_registration with the activation code for a new device"""
        self.duo.get_device_registration = MagicMock()
        with patch('builtins.input', side_effect=['push', 'new', 'CODE-aGVsbG8=']):
            self.duo.configure_mfa()
        self.duo.get_device_registration.assert_called_with('CODE-aGVsbG8=')
    ...
    @patch('synack.plugins.duo.RSA')
    def test_get_device_registration(self, mock_rsa):
        ...
        self.duo.get_device_registration(f'TESTCODE-{host_b64}')
    ...
    @patch('synack.plugins.duo.RSA')
    def test_get_device_registration_missing_padding(self, mock_rsa):
        ...
        self.duo.get_device_registration(f'CODE-{host_part}')
        self.assertEqual(self.duo._db.duo_host, 'a.com')

    @patch('synack.plugins.duo.RSA')
    def test_get_device_registration_stores_hotp_seed(self, mock_rsa):
        ...
        self.duo.get_device_registration(f'CODE-{host_b64}')
    ...
    def test_get_grant_token_push(self):
        ...
        self.duo.set_push_approved = MagicMock()
    ...
    def test_get_passcode_no_secret(self):
        """Should return None when no otp_secret is configured"""
        self.state._otp_secret = ''
        self.assertIsNone(self.duo.get_passcode())
    ...
    def test_get_prompt_finalize_progress_token(self):
        """Should exchange progress_token when finalize returns one"""
        self.duo._authkey = 'test_authkey'
        self.duo._base_url = 'https://api.duosecurity.com'
        self.duo._akey = 'test_akey'
        self.duo._get_grant_token = MagicMock()
        self.duo._utils.get_html_tag_value.return_value = 'csrf-token'
        res1 = MagicMock()
        res1.status_code = 200
        res1.json.return_value = {'response': {'url': 'https://login.synack.com/callback'}}
        res2 = MagicMock()
        res2.url = 'https://login.synack.com/callback?token=progress_token'
        res2.text = '<html></html>'
        self.duo._api.request.side_effect = [res1, res2]
        self.duo._get_prompt_finalize()
        self.assertEqual(self.duo._progress_token, 'progress_token')
        self.assertEqual(self.duo._xsrf, 'csrf-token')
        self.duo._get_grant_token.assert_called_once()
    ...
    def test_get_prompt_push_status_blocked(self, mock_sleep):
        """Should call set_push_approved on status_enum 15 and continue looping"""
        self.duo._authkey = 'test_authkey'
        self.duo._base_url = 'https://api.duosecurity.com'
        self.duo._akey = 'test_akey'
        ...
        self.duo.set_push_approved.assert_called()
    ...
    def test_get_status_awaiting_push(self, mock_sleep):
        """Should call set_push_approved on status_enum 13 when duo_akey is set"""
        self.duo._base_url = 'https://duofederal.com'
        self.duo._sid = 'test_sid'
        self.duo._txid = 'test_txid'
        self.duo._xsrf = 'test_xsrf'
    ...
    def test_get_status_push_sent(self, mock_sleep):
        """Should call set_push_approved on status_enum 56 and continue"""
        self.duo._base_url = 'https://duofederal.com'
        self.duo._sid = 'test_sid'
        self.duo._txid = 'test_txid'
        self.duo._xsrf = 'test_xsrf'
    ...
    def test_set_push_approved(self):
        ...
        self.duo._db.duo_host = 'api.duosecurity.com'
        self.duo._api.request.return_value.status_code = 200
        self.duo.set_push_approved()
        self.duo._api.request.assert_called_once()
        call_args = self.duo._api.request.call_args
        self.assertEqual(call_args[0][0], 'POST')
        self.assertIn('txn123', call_args[0][1])

    def test_set_push_approved_no_urgid(self):
        """Should skip transactions that have no urgid"""
        self.duo._get_push_transactions = MagicMock(return_value=[
            {'other_field': 'value'}
        ])
        self.duo._generate_push_signature = MagicMock(return_value='Basic sig')
        self.duo.set_push_approved(sleep=0)
        self.duo._api.request.assert_not_called()

    def test_set_push_approved_verified_push(self):
        ...
        self.duo._db.duo_host = 'api.duosecurity.com'
        self.duo._api.request.return_value.status_code = 200
        self.duo.set_push_approved(verified_push_code='244')
        call_args = self.duo._api.request.call_args
        self.assertEqual(call_args[1]['data']['step_up_code'], '244')

    def test_set_push_approved_verified_push_no_code_on_plain(self):
    ...
    def test_set_verified_push_code(self):
        ...
        self.duo._db.duo_host = 'api.duosecurity.com'
        self.duo._api.request.return_value.status_code = 200
        acted = self.duo.set_verified_push_code('244', attempts=1)
