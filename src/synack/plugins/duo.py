...
class Duo(Plugin):
    ...
    def _answer_transaction(self, transaction, answer='approve', verified_push_code=None):
        ...
        return self._api.request('POST', f'https://{self._state.duo_host}{path}',
                                 data=data,
                                 headers={'Authorization': signature, 'x-duo-date': now,
                                          'host': self._state.duo_host, 'txId': txid,
    ...
    def _configure_push(self):
        ...
        if choice == 'new':
            ...
            self.get_device_registration(code)
        else:
    ...
    def _generate_push_signature(self, method, path, now, data):
        rsa_key = RSA.import_key(self._state.duo_rsa_key)
        ...
        message = (now + '\n' + method + '\n' + self._state.duo_host.lower() + '\n' +
                   path + '\n' + canon_params).encode('ascii')
        h = SHA512.new(message)
        signature = pkcs1_15.new(rsa_key).sign(h)
        auth = ('Basic ' + base64.b64encode(
            (self._state.duo_pkey + ':' + base64.b64encode(signature).decode('ascii')).encode('ascii')
        ).decode('ascii'))
        return auth

    def get_device_registration(self, code):
    ...
    def _get_mfa_details(self):
        if self._state.otp_secret and not self._state.duo_akey:
            self._device = 'null'
    ...
    def _get_prompt_finalize(self):
        ...
        if res.status_code == 200:
            ...
            if exit_url:
                ...
                try:
                    self._grant_token = re.search('grant_token=([^&]*)', final.url).group(1)
                except AttributeError:
                    self._progress_token = re.search('token=([^&]*)', final.url).group(1)
                    self._xsrf = self._utils.get_html_tag_value('csrf-token', final.text)
                    if self._progress_token:
                        self._get_grant_token()
    ...
    def _get_push_transactions(self):
        ...
        res = self._api.request('GET', f'https://{self._state.duo_host}{path}',
                                query=data,
                                headers={'Authorization': signature, 'x-duo-date': now,
                                         'host': self._state.duo_host})
        if res.status_code == 200:
        ...
        for i in range(5):
            ...
            if res.status_code == 200:
                ...
                if status_enum == 5 or self._status == 'SUCCESS':  # Valid Code
                    ...
                    if self._state.duo_akey:
                        self.set_push_approved()
                elif status_enum == 15:  # Push Notification MFA Blocked
                    break
                elif status_enum == 44:  # Prior Code
                    count = int(self._state.otp_count or 0) + 5
                    self._state.otp_count = count
                    self._db.otp_count = count
                    break
    ...
    def _get_txid(self):
        ...
        if self._device and self._factor:
            ...
            if res.status_code == 200:
                ...
                if self._state.otp_secret:
                    count = int(self._state.otp_count or 0) + 1
                    self._state.otp_count = count
                    self._db.otp_count = count
    ...
    def set_push_approved(self, attempts=10, approvals=1, sleep=5,
    ...
    def set_verified_push_code(self, verified_push_code, attempts=10,
                               approvals=1, sleep=5):
        ...
        sleep -- seconds to wait between polling attempts
        """
        return self.set_push_approved(attempts=attempts,
                                      approvals=approvals,
                                      sleep=sleep,
